...
class StudentAuthThrottle(AnonRateThrottle): scope='student_auth'
...
def student_profile_payload(profile):
    ...
    data.update({'id':profile.pk,'email':profile.user.email,'first_name':profile.user.first_name,'last_name':profile.user.last_name,
...
def checklist_payload(item):
    return {'id':item.pk,'key':item.requirement.key,'title':item.requirement.title,'description':item.requirement.description,
        'required':item.requirement.required,'accepted_extensions':item.requirement.accepted_extensions,'status':item.status,
        'admin_note':item.admin_note,'visible_to_student':item.visible_to_student}

def document_payload(document):
    return {'id':document.pk,'document_type':document.document_type,'original_name':document.original_name,'file_size':document.file_size,
        'content_type':document.content_type,'status':document.status,'admin_note':document.admin_note,'uploaded_at':document.uploaded_at,
        'checklist_item':document.checklist_item_id}

def application_payload(application):
    course=application.course
    return {'id':application.pk,'reference_code':application.reference_code,'status':application.status,'status_label':application.get_status_display(),
        'student_note':application.student_note,'submitted_at':application.submitted_at,
        'created_at':application.created_at,'updated_at':application.updated_at,
        'course':{'id':course.pk,'title':course.title,'slug':course.slug,'degree':course.degree,'university':course.university.title} if course else None,
        'stages':[{'id':stage.pk,'key':stage.key,'title':stage.title,'description':stage.description,'status':stage.status,
            'status_label':stage.get_status_display(),'order':stage.order} for stage in application.stages.filter(visible_to_student=True)]}

def recommendation_payload(request):
    return {'id':request.pk,'kind':request.kind,'status':request.status,'student_question':request.student_question,
        'created_at':request.created_at,'updated_at':request.updated_at,'recommendations':[{'id':item.pk,'title':item.title,
            'course_id':item.course_id,'course_title':item.course.title if item.course else '',
            'rationale':item.rationale,'fit_score':item.fit_score} for item in request.recommendations.select_related('course')]}

def ai_course_recommendations(profile,request):
    if not settings.OPENAI_API_KEY: return False
    courses=list(m.Course.objects.filter(published=True,university__published=True).select_related('university').order_by('pk')[:250])
    catalog=[{'id':course.pk,'title':course.title,'university':course.university.title,'degree':course.degree,'discipline':course.discipline,
        'language':course.study_language,'location':course.study_location,'tuition':course.tuition_fee or course.tuition,'summary':course.summary[:500]} for course in courses]
    student={'previous_degree':profile.previous_degree,'previous_institution':profile.previous_institution,'field_of_study':profile.field_of_study,
        'grading_system':profile.grading_system,'overall_percentage':str(profile.overall_percentage or ''),'overall_gpa':str(profile.overall_gpa or ''),
        'graduation_year':profile.graduation_year,'english_test':profile.english_test,'english_score':profile.english_score,
        'target_degree':profile.target_degree,'target_disciplines':profile.target_disciplines,'preferred_cities':profile.preferred_cities,
...
def storj_client():
    ...
    return boto3.client('s3',aws_access_key_id=settings.STORJ_ACCESS_KEY,aws_secret_access_key=settings.STORJ_SECRET_KEY,
        endpoint_url=settings.STORJ_ENDPOINT,region_name='us-east-1',config=Config(signature_version='s3v4'))
...
def student_register(request):
    ...
    with transaction.atomic():
        ...
        profile=m.StudentProfile.objects.create(user=user,phone=data['phone'])
...
def student_me(request):
    ...
    return Response({'profile':student_profile_payload(profile),'checklist':[checklist_payload(item) for item in profile.checklist_items.select_related('requirement').filter(visible_to_student=True)],
        'documents':[document_payload(doc) for doc in profile.documents.all()],
        'applications':[application_payload(application) for application in profile.applications.select_related('course','course__university').prefetch_related('stages')],
        'recommendations':[recommendation_payload(item) for item in profile.recommendation_requests.prefetch_related('recommendations__course')]})
...
def student_profile(request):
    ...
    if request.method=='PATCH':
        ...
        first_name=data.pop('first_name',None); last_name=data.pop('last_name',None)
...
def student_profile_submit(request):
    profile=student_profile_for(request.user)
    required={'first_name':request.user.first_name,'last_name':request.user.last_name,'phone':profile.phone,'date_of_birth':profile.date_of_birth,
        'nationality':profile.nationality,'address':profile.address,'district':profile.district,'province':profile.province,
        'previous_degree':profile.previous_degree,'previous_institution':profile.previous_institution,'field_of_study':profile.field_of_study,
        'grading_system':profile.grading_system,'graduation_year':profile.graduation_year,'english_test':profile.english_test,
        'english_score':profile.english_score,'target_degree':profile.target_degree}
...
def student_document_presign(request):
    profile=student_profile_for(request.user)
    filename=str(request.data.get('file_name','')).strip(); content_type=str(request.data.get('content_type','application/octet-stream')).strip()
    ...
    checklist_id=request.data.get('checklist_item')
    item=None
    if checklist_id:
        item=get_object_or_404(m.StudentChecklistItem,pk=checklist_id,student=profile)
        allowed={x.strip().lower().lstrip('.') for x in item.requirement.accepted_extensions.split(',') if x.strip()}
    ...
    try:
        client=storj_client(); key=student_object_key(request.user,filename)
        upload_url=client.generate_presigned_url('put_object',Params={'Bucket':settings.STORJ_BUCKET,'Key':key,'ContentType':content_type},ExpiresIn=900,HttpMethod='PUT')
...
def student_documents(request):
    ...
    key=str(request.data.get('key','')); filename=str(request.data.get('file_name','')).strip(); content_type=str(request.data.get('content_type',''))
    prefix=f'private/student-documents/{request.user.pk}/'
    if not key.startswith(prefix) or '..' in key or not filename: return Response({'detail':'Invalid upload key.'},status=400)
    try:
        metadata=storj_client().head_object(Bucket=settings.STORJ_BUCKET,Key=key)
    except RuntimeError as error: return Response({'detail':str(error)},status=503)
    ...
    document=m.StudentDocument(student=profile,checklist_item=item,document_type=str(request.data.get('document_type','other')),file=key,original_name=filename,file_size=size,content_type=content_type or metadata.get('ContentType',''))
    document.save()
...
def student_applications(request):
    ...
    course_id=request.data.get('course'); course=get_object_or_404(m.Course,pk=course_id,published=True) if course_id else None
    reference=f'KA-{timezone.now().year}-{uuid.uuid4().hex[:8].upper()}'
    application=m.StudentApplication.objects.create(student=profile,course=course,reference_code=reference,status='submitted',submitted_at=timezone.now())
    for order,key,title in [(1,'profile','Profile review'),(2,'documents','Document verification'),(3,'application','Application preparation'),(4,'university','University decision')]:
        m.StudentApplicationStage.objects.create(application=application,key=key,title=title,order=order,status='current' if order==1 else 'pending')
...
def student_recommendations(request):
    ...
    item=m.StudentRecommendationRequest.objects.create(student=profile,kind=kind,student_question=str(request.data.get('student_question',''))[:5000])
    transaction.on_commit(lambda:notify(item))
